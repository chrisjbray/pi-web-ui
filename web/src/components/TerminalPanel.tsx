...
export function TerminalPanel({ chat, terminal, uiTerminalToolbar, onUiAction }: TerminalPanelProps) {
	...
	useEffect(() => {
		if (chat.ready) appSend({ type: "list_tmux_sessions" });
			...
			<div className="term-tmux-tree">
				<div className="term-tmux-head">
					<button
						...
						onClick={() => appSend({ type: "tmux_new_window", terminalId: tab.id, conversationId: tab.conversationId })}
					>
						<FiPlus />
					</button>
					<button
					...
					{tab.tmuxAdopted && (
						<button
							...
							onClick={() => {
								...
								appSend({ type: "tmux_detach", terminalId: tab.id, conversationId: tab.conversationId });
							}}
						>
							{t("tmuxDetachShort")}
						</button>
